//! Per-mount upstream credentials and custom headers.
//!
//! Every request a mount makes upstream — metadata, index, and artifact fetches
//! alike — carries these, because they are installed as the default headers of
//! that mount's HTTP client. Both of a mount's URLs (the crates.io index and its
//! downloads, PyPI's simple index and its file host) therefore get them; both
//! are URLs the operator configured for that mount.
//!
//! Two sources, per mount name:
//!
//! ```text
//! CHILLED_<NAME>_BASIC_AUTH_USERNAME / _PASSWORD    --upstream-basic-auth <name>=<user>:<pass>
//! CHILLED_<NAME>_HEADERS                            --upstream-header <name>=<header>: <value>
//! ```
//!
//! `<NAME>` is the mount name uppercased with `-` and `.` folded to `_`, so the
//! `gradle-plugins` mount reads `CHILLED_GRADLE_PLUGINS_BASIC_AUTH_USERNAME`.
...
pub struct UpstreamAuth {
    ...
    /// sensitive, so it prints as `Sensitive` rather than leaking through
    /// `Debug`.
    headers: HeaderMap,
