//! Marker construction and parsing. The marker carries the cutoff bucket as
//! well as the window length, so a filtered body is only equivalent to the
//! client's copy when both were produced for the same bucket.
