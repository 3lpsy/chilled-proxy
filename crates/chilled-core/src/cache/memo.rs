//! Memoization of filtered/rewritten response bodies, keyed by
//! `(key, source-validator, cutoff bucket)`; a mismatch on either tag
//! guarantees stale or time-shifted output is never returned.
...
/// Granularity (seconds) of the memo cutoff bucket. Hour-bucketing keeps the
/// key stable at the cost of ≤1h aging-in jitter on day-scale cooldowns.
...
impl FilteredMemo {
    ...
    /// source `validator` and cutoff `bucket`. An empty validator means the
    /// upstream offered neither an ETag nor a `Last-Modified`, so the memo
    /// declines rather than risk serving superseded bytes.
