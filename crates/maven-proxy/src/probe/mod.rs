...
//! probe failure records a first-seen stamp of *now* — fail-closed: the version
//! stays gated for a full cooldown window from first sight.
...
/// ones with bounded concurrency. A first-seen guess from an earlier failed
/// probe is retried while it still gates, so a transient failure does not hide
/// a version for a whole window. Returns `true` when anything changed.
///
/// An artifact with thousands of versions would take minutes probed one at a
/// time, which reads as a hang to the client waiting on the metadata.
...
fn needs_retry(times: &VersionTimes, version: &str, cutoff: u64) -> bool {
