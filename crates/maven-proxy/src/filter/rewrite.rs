//! The streaming metadata rewrite against the sidecar ages.
...
/// `ts > cutoff` (or unknown age) are dropped; `None` when nothing survives.
/// `versions` is the document's own version list (`list_versions` of the same
/// bytes) — passed in so the caller's parse is not repeated here.
